...
const Avatar = ({ src, fallback, ...props }: AvatarProps) => {
...
const AvatarSelect = ({ onChange }: AvatarUploadProps) => {
  ...
  const { run: fetchFileList, loading: loadingFileList } = useRequest(() => {
    return getFileList(iconListPageNumber + 1, 40, "avatar", "");
  }, {
    manual: true,
    onSuccess: (data) => {
...
export const AvatarUpload = ({ value, onChange, ...props }: AvatarUploadProps) => {
  ...
  return <>
    ...
    >
      <Upload
        ...
        {...props}
      >
        {avatar ? undefined : <AvatarSelect onChange={onChange} />}
      </Upload>
    </ImgCrop>
